...
src/
  ...
  pages/
    ...
    WelcomePage.jsx        Email-confirmation landing page at /welcome — no sidebar, accessible to logged-out users. On mount: calls supabase.auth.getSession(), checks email_confirmed_at, fires email_confirmed (localStorage-deduped per user per browser), then identifies user via identifyUser(). Sign-out on continue.
...
docs/
  ...
  posthog-pilot-dashboard.md  Setup instructions for 10 PostHog insights: acquisition funnel, confirmation conversion, activation funnel, time-to-activation, WAU, follow-up loop, CSV adoption, AI adoption, 7-day retention, error monitoring
...
| `email_confirmed` | WelcomePage on mount — fires after `supabase.auth.getSession()` confirms `email_confirmed_at` is set | none | Acquisition funnel — confirmation step |
