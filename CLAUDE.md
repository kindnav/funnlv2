...
| **CSV importer** | ✅ Import button on Contacts page opens a 3-step modal (upload → map → confirm). Mapping step is field-centric: each Funnl field owns a list of CSV columns (chips); multiple columns combine in chip order (e.g. First Name + Last Name → "John Smith"). One-column-one-field rule enforced by picker only showing unassigned columns. Live preview updates instantly as assignments change. Tags/skills: comma-separated cell values split into arrays. All-or-nothing bulk insert. `transformRow` has a clean seam for future AI pre-processing. Known limitations: no duplicate detection, CSV-only (no .xlsx), no cell-level editing (future). |
