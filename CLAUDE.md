...
| `csv_mapping_completed` | ImportContactsModal Step 2 "Next →" button | `{ fields_mapped: number, ai_assisted: boolean }` | CSV import funnel — mapping step completed |
| `csv_mapping_failed` | ImportContactsModal handleFile on parse error | `{ reason: 'no_header_detected'\|'no_data_rows' }` — controlled enum | CSV import error diagnostic |
| `post_import_action_clicked` | ImportContactsModal done screen CTAs | `{ action: 'log_outreach'\|'view_contacts' }` — controlled enum | Post-import activation path |
| `outreach_status_set` | ContactDetailPage handleLogInteraction when outreachStatus set | `{ status: 'awaiting_response'\|'responded'\|'meeting_booked'\|'no_response'\|'declined' }` — controlled enum | Outreach response tracking |
...
| `outreach_status` | text | Optional — one of: awaiting_response \| responded \| meeting_booked \| no_response \| declined. **Pending migration 20260721000000_add_outreach_status.sql — do not use in production until Naveen applies it.** |
| `created_at` | timestamptz | Auto-set |
...
| **Pilot: Robust CSV parsing + per-contact AI category inference** | ✅ (branch review/pilot-csv-intelligence-outreach-status) Two-pass header detection logic extracted to `src/lib/csvHeaderDetect.js`; enhanced HEADER_MAP with 'display name', 'current employer', 'title', 'profile link', 'categories', 'groups' aliases; SCORE_EXTRA extended with location/twitter/city/state/country/industry as score-only columns (not assigned). `__parsed_extra` defensive filter in `indexedHeaders`. **AI inference is per-contact, not global:** `ai-map-csv` handles column header mapping only (no category inference); new `ai-categorize-contacts` Edge Function accepts normalized contact rows (non-PII only: row_id, company, role, how_met, relationship_note, existing_tags, existing_relationship_type — no names, emails, or LinkedIn URLs) and returns per-contact `suggested_tags` + `suggested_relationship_type`; batches all contacts in one Haiku call (max 200); suggestions pre-accepted but individually toggleable in Step 3; user can import immediately without waiting for or accepting suggestions; Silent fallback if AI call fails. `goToConfirm()` computes `normalizedContacts` with stable `_rowId`s once and kicks off `runContactCategorization` (non-blocking) for Pro users. `handleImport()` strips `_rowId` and merges per-contact `acceptedByRow` into the final insert. 36 unit tests (node tests/csv-header-detect.test.js) including 9 new alias/edge-case tests. |
| **Pilot: Post-import activation handoff** | ✅ (branch review/pilot-csv-intelligence-outreach-status) Done screen in ImportContactsModal now shows "What would you like to do next?" panel with "Log recent outreach" (primary — navigates to first imported contact with `openInteractionForm: true` Router state) and "View all contacts" (secondary). Footer "Done" demoted to ghost close link. `post_import_action_clicked` event fires on each CTA. |
| **Pilot: Outreach response status** | ✅ (branch review/pilot-csv-intelligence-outreach-status, pending migration) Migration `20260721000000_add_outreach_status.sql` adds `outreach_status text` to interactions (DO NOT apply without approval). ContactDetailPage: log and edit forms gain optional outreach status select (awaiting_response \| responded \| meeting_booked \| no_response \| declined). Timeline rows show colored `OutreachStatusBadge` beside interaction type when set. `outreach_status_set` event fires on log. |
| **Pilot: Light / Dark / System theme** | ✅ (branch review/pilot-csv-intelligence-outreach-status) `src/lib/theme.js` manages persistence and application. `initTheme()` called before React render in `main.jsx`. `index.css` adds CSS variable overrides for light theme under `@media (prefers-color-scheme: light)` and `:root[data-theme]` selectors — named token utilities (bg-card, text-hi, etc.) respond automatically. Settings page gains System/Light/Dark toggle that takes effect instantly. AddContactDrawer `bg-[#0E0E12]` replaced with `bg-card`. Note: arbitrary-value border/opacity classes remain dark-only in this iteration. |
