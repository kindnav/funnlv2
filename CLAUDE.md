...
| `pro_trial_started` | WelcomePage.jsx — fires after `start_my_pro_trial()` RPC returns `started_now: true` | none | Trial activation (fires exactly once per user, on first email confirmation) |
...
| **Product analytics (PostHog)** | ✅ 23 events total (8 core + 2 Phase 1 + 3 Phase 2A + 2 Phase 3 + 2 Phase 4 + 4 Pilot + 1 AI reliability + 1 Pro trial). Core: user_signed_up, first_contact_added, contact_added, interaction_logged, followup_set, csv_import_used, ai_assistant_used, ai_fill_used. Phase 1: landing_cta_clicked, signup_started. Phase 2A: activation_checklist_viewed, activation_step_completed, activation_completed. Phase 3: followup_completed, followup_snoozed. Phase 4: email_confirmed, user_signed_in. Pilot: csv_mapping_completed, csv_mapping_failed, post_import_action_clicked, outreach_status_changed. AI reliability: ai_assistant_failed. Pro trial: pro_trial_started. DOM autocapture disabled; $pageview and $pageleave remain enabled. Behavior only — no contact content. Users identified by Supabase ID. |
