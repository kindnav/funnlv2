...
| **Pilot: Robust CSV parsing + per-contact AI category inference** | ✅ (branch review/pilot-csv-intelligence-outreach-status) Two-pass header detection logic extracted to `src/lib/csvHeaderDetect.js`; HEADER_MAP enhanced with 'display name', 'current employer', 'title', 'categories', 'groups' aliases; 'profile link' intentionally excluded from HEADER_MAP (too generic — only mapped to linkedin_url when value-sniffed in ImportContactsModal); SCORE_EXTRA extended with location/twitter/city/state/country/industry as score-only columns (not assigned). `__parsed_extra` defensive filter in `indexedHeaders`. **AI inference is per-contact, not global:** `ai-map-csv` handles column header mapping only (no category inference); new `ai-categorize-contacts` Edge Function accepts minimized contact context (row_id, company, role, how_met, relationship_note, existing_tags, existing_relationship_type — names, emails, LinkedIn URLs excluded) and returns per-contact `suggested_tags` + `suggested_relationship_type` + `confidence`; batched at 20 contacts per call with max 2 concurrent batches (NOT one monolithic call); **confidence-based preselection**: high confidence → AI tags and relType pre-accepted; medium/low → shown unselected for review. User can import immediately without waiting for or accepting suggestions. Stale-request guard (inferenceRunIdRef) cancels in-flight AI on mapping change or file upload. Mapping-signature cache prevents re-running AI when user navigates Back/Confirm without changing columns. Silent fallback if AI call fails. `goToConfirm()` computes `normalizedContacts` with stable `_rowId`s once and kicks off `runContactCategorization` (non-blocking) for Pro users. `handleImport()` strips `_rowId` and merges per-contact `acceptedByRow` into the final insert. Custom tags: max 50 chars, max 5 total (AI+custom), dedup enforced in UI and capped in import merge. 37 tests in tests/csv-header-detect.test.js + 31 tests in tests/ai-helpers.test.js + 22 tests in tests/theme.test.js = 90 tests total. |
...
| **Pilot: Outreach response status** | ✅ (branch review/pilot-csv-intelligence-outreach-status, pending migration) Migration `20260721000000_add_outreach_status.sql` adds `outreach_status text` to interactions with named CHECK constraint `interactions_outreach_status_check` in a guarded DO block (DO NOT apply without approval). ContactDetailPage: Email/Message log form auto-enables outreach tracking (default "Awaiting response", opt-out checkbox); Call/Other shows manual select with label "Track the response manually. Automatic inbox syncing is not enabled." (Funnl does NOT read Gmail or any inbox); Coffee chat/Event hide outreach tracking. Edit form always shows select. Timeline rows show colored `OutreachStatusBadge` beside interaction type when set. Latest outreach status shown in details card (derived from loaded interactions — no N+1 query). `outreach_status_changed` event fires with `{ status: enum\|'cleared', context: 'new_interaction'\|'edit_interaction' }`. |
