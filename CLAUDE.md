...
| **Pilot: Robust CSV parsing + AI category inference** | ✅ (branch review/pilot-csv-intelligence-outreach-status) Two-pass header detection logic extracted to `src/lib/csvHeaderDetect.js`; enhanced HEADER_MAP with 'display name', 'current employer', 'title', 'profile link', 'categories', 'groups' aliases; SCORE_EXTRA extended with location/twitter/city/state/country/industry as score-only columns (not assigned). `__parsed_extra` defensive filter in `indexedHeaders`. `ai-map-csv` Edge Function gains `infer_categories: true` flag: when set, Claude also suggests `suggested_tags` (≤5, lowercased, ≤50 chars each) and `suggested_relationship_type` (validated against ALLOWED_REL_TYPES); both sanitized server-side. ImportContactsModal Step 3 shows editable AI suggestions panel for Pro users: tag chips and relationship type button toggle on/off; pre-selected by default. Accepted tags merged with CSV-mapped tags (deduped); accepted relType applied only when CSV didn't already set one. 36 unit tests (node tests/csv-header-detect.test.js) including 9 new alias/edge-case tests. |
